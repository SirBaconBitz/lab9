...
void Sort<T>::_quickSort(T** items, int first, int last, int (*compare) (T* one, T* two))
{
   ...
   if (first < last)
   {
      ...
      Sort<T>::_quickSort(items, first, pivotIndex-1, compare);

      Sort<T>::_quickSort(items, pivotIndex+1, last, compare);
   }  
}  
...
int Sort<T>::partition(T** items, int first, int last, int (*compare) (T* one, T* two))
{
   ...
   {
      int compareItems = (*compare) (items[i], items[first]);
